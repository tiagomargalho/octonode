...
(function() {
  ...
  Repo = (function() {
    ...
    Repo.prototype.issuesWithFilter = function(milestone, state, since, cb) {
      var url;
      if (milestone == null) {
        milestone = "*";
      }
      ...
      url = "/repos/" + this.name + "/issues?milestone=" + milestone + "&state=" + state;
      if (since) {
        url += "&since=" + since;
      }
      return this.client.get(url, function(err, s, b) {
        ...
        if (s !== 200) {
          return cb(new Error("Repo issuesWithFilter error"));
        } else {
          return cb(null, b);
        }
      });
    ...
    Repo.prototype.commentsSince = function(since, cb) {
      return this.client.get("/repos/" + this.name + "/issues/comments", function(err, s, b) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Repo commentsSince error"));
        } else {
          return cb(null, b);
        }
      });
    ...
    Repo.prototype.commentsForIssue = function(issueId, cb) {
      return this.client.get("/repos/" + this.name + "/issues/" + this.issueId + "/comments", function(err, s, b) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Repo commentsForIssue error"));
        } else {
          return cb(null, b);
        }
      });
